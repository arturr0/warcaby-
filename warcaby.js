...
const socket = io.connect('https://mango-flash-sherbet.glitch.me/warcaby');
////////////////console.log("warcaby");
let Player;
let room = "";
let playerName;
...
function mouseClicked() {
  ...
  for (let i = 0; i < Pawns.length; i++) {
    let p = Pawns[i];
    if ((!killedOptMode && !killersOptMode) && ((p.isRed && !Greenturn && Player == 1) || (!p.isRed && Greenturn  && Player == 2)) && p.live &&
    ...
    for (let i = 0; i < killersOpt.length; i++) {
      ...
      if (((killersOpt[i][3] && !Greenturn && Player == 1) || (!killersOpt[i][3] && Greenturn  && Player == 2)) &&
          X > killersOpt[i][5] - 32 && X < killersOpt[i][5] + 32 && Y > killersOpt[i][6] - 32 && Y < killersOpt[i][6] + 32) {
            ...
            //kill();
            let firstKill = [];
            firstKill.push(killConditions[0])
            killOpt(firstKill);
            stepKill(firstKill);
...
//killConditions.push([k, j, i, Pawns[k].isRed, Greenturn, Pawns[k].rectCenter, Pawns[k].rectCenterY, Pawns[j].rectCenter, Pawns[j].rectCenterY]);
    if (killedOptMode) {
      for (let i = 0; i < killersOpt.length; i++) {
        
        if (((killersOpt[i][3] && !Greenturn && Player == 1) || (!killersOpt[i][3] && Greenturn  && Player == 2)) &&
            X > killersOpt[i][7] - 32 && X < killersOpt[i][7] + 32 && Y > killersOpt[i][8] - 32 && Y < killersOpt[i][8] + 32) {
              //////////////////////////console.log("click");
              for (let j = 0; j < killersOpt.length; j++)
                Pawns[killersOpt[j][1]].killed = false;
              ////////////////////////////////////////////////////////////////////////////////console.log(killersOpt[i]);
              let killedSelected = killersOpt[i][1];
              //////////////////////////////////////////////console.log(pawnSelected);
              for (let j = 0; j < killConditions.length; j++)
                if (killConditions[j][1] != killedSelected) {
                  //////////////////////////////////////////////console.log(killConditions[j][0]);
                  killConditions.splice(j,1); 
              ...
              killOpt(firstKill);
              stepKill(firstKill);
              socket.emit('killed mode', killersOptMode, Pawns, room);
          }
 ...
 if (killConditions.length > 1) {
  // Start from 1 to avoid comparing the element with itself
   ...
   if (killConditions[i][0] != killConditions[1][0] && 
     ...
     //////////////////console.log(`killer1: ${killConditions[i][0]} killer2: ${killConditions[1][0]} killed1: ${killConditions[i][1]} killed2: ${killConditions[1][1]}`);
     killedOptMode = true;
     Pawns[killConditions[i][1]].killed = true;
     Pawns[killConditions[1][1]].killed = true;
     killersOpt.push(killConditions[i]);
     killersOpt.push(killConditions[1]);
...
function killOpt(killmode) {
  console.log(killmode)
    for (let i = 0; i < killmode.length; i++)
        if ((!killersOptMode && !killedOptMode) && Pawns[killmode[i][1]].live) {
          killSwitch(killmode[i][0],killmode[i][1],killmode[i][2],killmode[i][3]);
          console.log(killmode[0][0]);
        }
...
function killSwitch(winner, looser, newBoard, player) {
  ...
  if ((!killersOptMode && !killedOptMode) && Pawns[looser].live) {
...
if ((!killersOptMode && !killedOptMode)) { 
...
for (let i = 0; i < killmode.length; i++)
  ...
  if (killmode.length > 1) check = true;
  else check = false;
  killmode.splice(i, 1);
  
  socket.emit('state', Board, Pawns, Greenturn, check, current, killmode, room);
