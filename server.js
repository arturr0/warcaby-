...
warcabyNamespace.on('connection', (socket) => {
    ...
    socket.on('state', function(Board, Pawns, Greenturn, check, current, killConditions, room) {
        ...
        warcabyNamespace.to(room).emit('new state', BOARD, PAWNS, TURN, PLAY, CHECK, KILL);
      });
      ...
      socket.on('killed mode', function(killedOptMode, Pawns, room) {
        KILLED_MODE = killedOptMode;
        PAWNS = Pawns;
        warcabyNamespace.to(room).emit('update killed mode', KILLED_MODE, PAWNS);  
      });
